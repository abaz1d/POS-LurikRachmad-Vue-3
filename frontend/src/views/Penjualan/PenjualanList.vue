<template>
  ...
  <Modal size="modal-xl" :show="isInvoice" @hidden="isInvoice = false">
    ...
    <ModalBody>
      <PrintInvoice :prints="Penjualan.prints" :no_invoice_show="no_invoice_show" :total_harga_jual="total_harga_jual"
  ...
  <Modal size="modal-xl" backdrop="static" :show="isEdit" @hidden="isEdit = false">
    ...
    <ModalFooter class="text-right bottom-0 relative z-50 rounded-md sm:border-t-2 border-t-4 btm sm:btm-">
      ...
      <button type="button" @click="editPenjualan()" class="object-left btn btn-primary w-32"
...
export default {
  ...
  methods: {
    ...
    async openEditModal(penjualan) {
      console.log("openEditModal", penjualan);
      this.no_invoice = penjualan.no_invoice;
      this.waktu = penjualan.tanggal_penjualan;
      this.total_harga_global = penjualan.total_harga_jual;
      this.total_bayar_global = penjualan.total_bayar_jual;
      this.kembalian = penjualan.kembalian_jual;
      this.Penjualan.readDetailPenjualan(penjualan.no_invoice).then((data) => {
        this.isEdit = true;
      }).catch((e) => console.error(e));
