      ...
      <Modal size="modal-xl" backdrop="static" :show="modal_utama" @hidden="modal_utama = false">
        <ModalHeader class="relative top-0 z-50 rounded-md border-b-2">
          <h2 class="hidden lg:block font-medium text-base mr-auto">
            <p class="mx-auto" v-if="isEdit">Edit BarangMasuk {{ no_invoice }}</p>
            <p class="mx-auto" v-else>Tambah BarangMasuk</p>
          </h2>
                          ...
                          <div class="sm:col-span-9 col-span-12 mb-5">
                            ...
                            <div class="flex w-full">
                              ...
                              <TomSelect v-model="item_select" class="w-full" required>
                                ...
                                <option v-for="varian in BarangMasuk.varians" :key="varian.id_varian" :varian="varian"
              ...
              <div class="col-span-12 flex-col-reverse">
                <div class="intro-y box">
                  <div class="flex items-center px-5 py-2 border-b border-slate-200/60 dark:border-darkmode-400">
                    <h2 class="font-medium text-base mr-auto">Detail BarangMasuk</h2>
                  </div>
                  <div class="px-2">
                    <div class="col-span-12 overflow-auto w-full h-56">
                      <table class="table table-hover mt-2">
                        ...
                        <tbody>
                          <tr v-for="detail in BarangMasuk.pembelianDetail" :key="detail.id_barang" :detail="detail">
      ...
      <form id="tabulator-html-filter-form" class="xl:flex sm:mr-auto">
        <div class="sm:flex items-center sm:mr-4">
          ...
          <select id="tabulator-html-filter-field" v-model="filter.field"
            ...
            <option value="tanggal_pembelian">Tanggal BarangMasuk</option>
...
const startTransaction = () => {
  BarangMasuk.startTransaction().then((data) => {
...
const addItem = () => {
  BarangMasuk.addDetailPembelian(
    no_invoice.value,
    item_select.value,
    qty_select.value
  ).then((data) => {
          ...
          const a = dom(`
            <div class="flex justify-center items-center">
                <a class="flex items-center text-primary whitespace-nowrap mr-5"
                  href="javascript:;"
                >
                  <i data-lucide="package-minus" class="w-4 h-4 mr-1"></i>${cell.getData().id_outlet_pengirim}
                </a>
                <i data-lucide="arrow-right" class="w-4 h-4 m-2 -ml-3"></i>
                <a class="flex items-center text-primary whitespace-nowrap"
                  href="javascript:;"
                >
                  <i data-lucide="package-plus" class="w-4 h-4 mr-1"></i>${cell.getData().id_outlet_penerima}
                </a>
              </div>
