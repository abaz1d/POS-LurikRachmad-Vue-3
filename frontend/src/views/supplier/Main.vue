  ...
  <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    ...
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
      <button class="btn btn-primary shadow-md mb-3 mr-2 pr-5" @click="modal_utama = true">
        ...
        <p class="hidden xl:block mr-1">Supplier</p> Baru
      </button>
      <!-- BEGIN: Modal Content -->
      <Modal backdrop="static" :show="modal_utama" @hidden="modal_utama = false">
        ...
        <ModalBody class="grid grid-cols-12 gap-4 gap-y-3">
          <form @submit.prevent="isEdit ? updateSupplier() : addSupplier()" id="supplierForm" class="col-span-12">
            <div class="col-span-12 mb-5">
              <label for="pos-form-1" class="form-label mb-1">Nama Supplier</label>
              <input id="pos-form-1" type="text" class="form-control flex-1" placeholder="Masukan Nama Supplier"
                v-model="nama_supplier" required />
            </div>
            <div class="col-span-12 mb-5">
              ...
              <textarea id="pos-form-5" class="form-control" placeholder="Masukan Alamat Supplier"
                v-model="alamat_supplier" required />
            </div>
            <div class="col-span-12 mb-5">
              <label for="pos-form-1" class="form-label mb-1">Telepon Supplier</label>
              <input id="pos-form-1" type="number" class="form-control flex-1" placeholder="Masukan Telepon Supplier"
                v-model="telepon_supplier" required />
            </div>
            <div class="col-span-12 mb-5">
              <label for="pos-form-1" class="form-label mb-1">Email Supplier</label>
              <input id="pos-form-1" type="text" class="form-control flex-1" placeholder="Masukan Email Supplier"
                v-model="email_supplier" required />
            </div>
          </form>
        </ModalBody>
        <ModalFooter class="text-right">
          <button type="button"
            @click="modal_utama = false; id_supplier = ''; nama_supplier = ''; alamat_supplier = ''; telepon_supplier = ''; email_supplier = ''; isEdit = false;"
            class="btn btn-outline-secondary w-32 mr-1">
            Cancel
          </button>
          <button type="submit" form="supplierForm" class="btn btn-primary w-32">
            Simpan
          </button>
        </ModalFooter>
      </Modal>
    ...
    <ModalBody class="p-0">
      <div class="p-5 text-center">
        ...
        <div class="text-slate-500 mt-2">
          Anda yakin ingin menghapus data <b>{{ nama_supplier }}</b> ? <br />Data
      ...
      this.tabulator = new Tabulator(this.$refs.tableRef, {
        ...
        columns: [
          ...
          },
          {
            ...
            formatter(cell) {
              return `<div>
                <div class="font-medium whitespace-nowrap">${cell.getData().nama_supplier
                }</div>
              </div>`;
            },
          },
          ...
          },
          {
            ...
            formatter(cell) {
              return `<div>
                <div class="font-medium whitespace-nowrap">${cell.getData().telepon_supplier
                }</div>
              </div>`;
            },
          },
          {
            ...
            formatter(cell) {
              return `<div>
                <div class="font-medium whitespace-nowrap">${cell.getData().email_supplier
                }</div>
              </div>`;
            },
          },
