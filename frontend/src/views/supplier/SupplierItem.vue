    ...
    <td class="table-report__action w-56" v-else>
      <div class="flex justify-center items-center">
        ...
        <button
          ...
          type="button"
          @click="
            $emit('openModal', supplier.id_supplier, supplier.nama_supplier)
          "
        >
    ...
    updateSupplier(id_supplier) {
      try {
        this.Supplier.updateItem({
          ...
          alamat_supplier: this.alamatSupplier,
          telepon_supplier: this.teleponSupplier,
          email_supplier: this.emailSupplier,
        });
        this.isEdit = false;
      } catch (error) {
