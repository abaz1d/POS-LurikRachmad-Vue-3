<template>
	<Alert v-if="alert" class="hover:animate-none md:animate-bounce animate-pulse alert-primary md:mb-2 m-5"><b> Klik pada nomor invoice untuk melihat data tabel detail pembelian dan mencetak
		invoice</b> <XCircleIcon @click="alert = false" class="block absolute stroke-2 stroke-red-500 bg-white rounded-full h-6 w-6 -right-3 -top-3 cursor-pointer" /></Alert>
	<table class="table table-report table-hover mt-2">
		...
		<tbody>
			<BarangKeluarItem v-for="(barangKeluar, index) in barangKeluars" :key="barangKeluar.no_invoice"
				:barangKeluar="barangKeluar" @openDeleteModal="openDeleteModal" @openInvoice="openInvoice" />
		</tbody>
	</table>
		...
		<ModalBody class="p-0">
			<div class="p-5 text-center">
				...
				<div class="text-slate-500 mt-2">
					Do you really want to delete <b>{{ no_invoice }} </b> ? <br />This process cannot
					be undone.
				</div>
			</div>
			<div class="px-5 pb-8 text-center">
				...
				<button type="button" class="btn btn-danger w-24" @click="deleteBarangKeluar()">Delete</button>
			</div>
		</ModalBody>
	</Modal>
	...
	<Modal size="modal-xl" :show="isInvoice" @hidden="isInvoice = false">
		<ModalHeader>
			<h2 class="font-medium text-base mr-auto">Transaksi No Invoice <b>{{ no_invoice }}</b> </h2>
		...
		<ModalBody>
			<div class="intro-y box overflow-hidden mt-2">
				<div class="flex flex-col lg:flex-row pt-10 px-5 sm:px-20 sm:pt-20 lg:pb-20 text-center sm:text-left">
					<div class="font-semibold text-primary text-3xl">INVOICE <br>
						<div class="text-xl dark:text-secondary text-dark font-medium">{{ no_invoice }}</div>
					</div>
