...
module.exports = function (db) {
    router.get('/', isLoggedIn, async function (req, res, next) {
        try {
            ...
            const { rows } = await db.query('SELECT bm.*, ot.nama_outlet AS penerima, op.nama_outlet AS pengirim FROM barang_masuk bm LEFT JOIN outlet ot ON bm.id_outlet_penerima = ot.id_outlet LEFT JOIN outlet op ON bm.id_outlet_pengirim = op.id_outlet WHERE id_outlet_penerima = $1',[id_outlet]);
